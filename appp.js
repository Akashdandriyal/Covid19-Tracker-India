...
const vaccineData = require('./vaccine');
...
appp.get("/covid19Cases", function(req, res){
    try {
        const url = "https://api.covid19api.com/summary";
        https.get(url, function(response){
            let body = "";
...
appp.get("/covid19Vaccination",async function(req, res){
    try {
        let data = await vaccineData.getVaccine();
        res.render("covidVaccination", {
            vaccineData: data.countries,
            date: data.date,
            time: data.time,
            worldVaccination: data.countries[data.countries.length - 2]
        });
    } catch(err) {
        res.render("error");
    }
